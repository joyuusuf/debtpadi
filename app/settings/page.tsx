  ...
  saveAvatar(ev.target?.result as string);
            ...
            {activeTab === "security" && (
              <div className="bg-white border border-ink-100 rounded-2xl p-4 sm:p-6">
                ...
                <div className="space-y-2 sm:space-y-3">
                  {[
                    { label: "Change password",            href: "/settings/security/change-password", danger: false },
                    { label: "Two-factor authentication",  href: "/settings/security/two-factor",       danger: false },
                    { label: "Active sessions",            href: "/settings/security/sessions",         danger: false },
                    { label: "Delete account",             href: "/settings/security/delete-account",   danger: true  },
                  ].map(item => (
                    <a
                      key={item.href}
                      href={item.href}
                      className={`w-full flex items-center justify-between px-3 sm:px-4 py-3 sm:py-3.5 rounded-xl border transition-colors text-left group ${
                        item.danger
                          ? "border-ink-100 hover:border-coral-200 hover:bg-coral-50"
            ...
            {activeTab === "app" && (
              <div className="bg-white border border-ink-100 rounded-2xl p-4 sm:p-6">
                ...
                <div className="space-y-2 sm:space-y-3">
                  {["Currency display", "Language", "Date format", "Offline data sync", "Clear local cache"].map((item, i) => (
                    <button
                      key={i}
